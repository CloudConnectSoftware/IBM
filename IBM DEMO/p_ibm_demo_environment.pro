...
:-
	...
	(
		...
		buyer_party__lookup( _, Party_Lookup, _ ),
...
%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
i_analyse_invoice_fields_first:- i_analyse_supplier_vat_number___.
%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
i_analyse_supplier_vat_number___
%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
:-
	result( _, invoice, supplier_vat_number, Vat ),

	(
		string_to_upper( Vat, Vat_U ),

		buyer_party__lookup( _, _, Vat_Lookup ),

		string_to_upper( Vat_Lookup, Vat_U )

		;

		sys_retractall( result( _, invoice, supplier_vat_number, _ ) ),

		trace( [ `Vat number invalid, removed`, Vat ] )
...
%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
:-
	not( q_enquire_form( `customer_intervention_form`, _ ) ),

	result( _, invoice, supplier_vat_number, VAT ),

	string_to_upper( VAT, VAT_U ),
	
	buyer_party__lookup( Vendor_id, Party_Lookup, VAT_lookup ),

	string_to_upper( VAT_lookup, VAT_U ),

	result( _, invoice, supplier_vat_number, _ ),

	trace( [ `Buyer Party from lookup`, Party_Lookup ] ),

	trace( [ `Vendor_id from lookup`, Vendor_id ] ),
	
	trace( [ `Buyer Party from lookup`, VAT_U ] ),
	
	sys_retractall( result( _, invoice, buyers_code_for_supplier, _ ) ),

	assertz_derived_data( invoice, buyers_code_for_supplier, Vendor_id, i_analyse_vendor_id___ ),

	assertz_derived_data( invoice, supplier_party, Party_Lookup, i_analyse_vendor_id___ ),

	assertz_derived_data( invoice, sender_name, Party_Lookup, i_analyse_vendor_id___ ),
